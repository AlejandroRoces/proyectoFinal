    ...
    <div id="main-wrapper" data-layout="vertical" data-navbarbg="skin6" data-sidebartype="full"
                    ...
                    <ul id="sidebarnav">
                        ...
                        <<li class="sidebar-item">
                            <a class="sidebar-link waves-effect waves-dark sidebar-link" href="familiasDashBoard_Mensajes.php" aria-expanded="false">
                                ...
                                // Suponiendo que el usuario autenticado tiene su correo en $_SESSION['email']
                                session_start();
                                $email_usuario = $_SESSION['email'];

                                try {
                                    $conn = conectarDB();
                                    $sql = "SELECT COUNT(*) AS total FROM mensajes WHERE destinatario = :email AND leido = 0";
                                    ...
                                    if ($total_mensajes > 0) {
                                        echo "<span class='badge badge-danger'>$total_mensajes</span>";
                                    }
        ...
        <table border="1">
            ...
            <?php foreach ($mensajes as $mensaje) : ?>
                <tr>
                    <td><?= htmlspecialchars($mensaje['destinatario']) ?></td>
                    <td><?= nl2br(htmlspecialchars($mensaje['mensaje'])) ?></td>
                    <td><?= $mensaje['leido'] ? "Leído" : "No leído" ?></td>
                    <td><?= $mensaje['fecha'] ?></td>
                </tr>
            <?php endforeach; ?>
        </table>
